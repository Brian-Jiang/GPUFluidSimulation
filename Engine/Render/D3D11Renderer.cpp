	...
	:IRenderer(context, width, height)
{
	...
	UINT adapterId = 0;
	while (pFactory->EnumAdapters(adapterId, &pAdapter) != DXGI_ERROR_NOT_FOUND) {
		DXGI_ADAPTER_DESC desc;
		pAdapter->GetDesc(&desc);

		if (wcsstr(desc.Description, L"NVIDIA") != nullptr) {
			//std::wcout << L"Selected Adapter: " << desc.Description << std::endl;
			//*selectedAdapter = pAdapter.Detach(); // Assign selected adapter
			break;
		}
...
#		endif
	...
	dsvDesc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT; // Depth-Stencil format
	dsvDesc.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2D;
	dsvDesc.Texture2D.MipSlice = 0;
	m_pD3DDevice->CreateDepthStencilView(
		m_pDepthStencilBuffer, &dsvDesc, &m_pDepthStencilView);

	D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
	srvDesc.Format = DXGI_FORMAT_R24_UNORM_X8_TYPELESS; // Depth format for SRV
	srvDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
	srvDesc.Texture2D.MipLevels = 1;

	m_pD3DDevice->CreateShaderResourceView(m_pDepthStencilBuffer, &srvDesc, &m_pDepthStencilSRV);
...
void D3D11Renderer::setRenderTargetsAndViewportWithDepth(TextureGPU *pDestColorTex, TextureGPU *pDestDepthTex, bool clearRenderTarget, bool clearDepth)
{
	...
	ID3D11DepthStencilView* dsv = pDestDepthTex ? pDestDepthTex->m_DepthStencilView : m_pDepthStencilView;
	m_pD3DContext->OMSetRenderTargets(1, renderTargets, dsv);
	static float x = 0;
#if 0
	x = fmodf(x + 0.01, 1.0f);
#endif
	if (pDestColorTex != 0)
	{
		...
		if (clearDepth)
			m_pD3DContext->ClearDepthStencilView(dsv, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
	}
		else
	{
		D3D11_VIEWPORT vp;
