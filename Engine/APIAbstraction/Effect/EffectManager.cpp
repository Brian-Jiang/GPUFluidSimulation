...
EffectManager::EffectManager(PE::GameContext &context, PE::MemoryArena arena)
    ...
    , m_firstPassTextureGPU(context, arena)
{
...
#	elif APIABSTRACTION_D3D11
		...
		ID3D11Buffer *pBuf;
		{
			...
			cbDesc.ByteWidth = sizeof(SetFluidConstantsShaderAction::Data);
			hr = pDevice->CreateBuffer(&cbDesc, NULL, &pBuf);
			assert(SUCCEEDED(hr));
			SetFluidConstantsShaderAction::s_pBuffer = pBuf;
			m_cbuffers.add("cbFluidConstants", pBuf);
		}

		{
			...
			bufferDesc.BindFlags = D3D11_BIND_SHADER_RESOURCE; //D3D11_BIND_UNORDERED_ACCESS allows writing to resource and reading from resource at the same draw call
			bufferDesc.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;
			bufferDesc.ByteWidth = sizeof(float) * 2 * FluidSetBufferGPU::c_maxForcePositions;
			bufferDesc.StructureByteStride = sizeof(float) * 2;

			float* initData = new float[2 * FluidSetBufferGPU::c_maxForcePositions];
			for (int i = 0; i < FluidSetBufferGPU::c_maxForcePositions; ++i) {
			...
			HRESULT hr = pDevice->CreateBuffer(&bufferDesc, &vinitData, &SetFluidForcesShaderAction::s_pBuffer);
			...
			srvDesc.Buffer.NumElements = FluidSetBufferGPU::c_maxForcePositions;

			hr = pDevice->CreateShaderResourceView(SetFluidForcesShaderAction::s_pBuffer, &srvDesc, &SetFluidForcesShaderAction::s_pSRV);
        ...
        FluidSetBufferGPU::createBuffers(*m_pContext, 512, 512);
#	endif
}
...
void EffectManager::buildFullScreenBoard()
{
	...
	m_hColoredMinimalMeshTech = getEffectHandle("ColoredMinimalMesh_OneTexture_Tech");
	m_hFluidVelocityEffect = getEffectHandle("Fluid_FullScreen_PS_Tech");
}
...
{
	m_pContext->getGPUScreen()->setRenderTargetsAndViewportWithDepth(m_hGlowTargetTextureGPU.getObject<TextureGPU>(), 0, true, false);
	m_pCurRenderTarget = m_hGlowTargetTextureGPU.getObject<TextureGPU>();
}

void EffectManager::setRTForFirstPass() {
    m_pContext->getGPUScreen()->setRenderTargetsAndViewportWithDepth(&m_firstPassTextureGPU, 0, true, true);
}
...
void EffectManager::unbindDepth() {
	m_pContext->getGPUScreen()->setRenderTargetsAndViewportWithNoDepth(m_hGlowTargetTextureGPU.getObject<TextureGPU>(), false);
}
...
void EffectManager::drawFullScreenQuad()
{
	...
	// todo use detailed mesh
	TextureGPU* pTex = &m_firstPassTextureGPU;
...
void EffectManager::drawFluidVelocity()
{
	Effect& curEffect = *m_hFluidVelocityEffect.getObject<Effect>();
	...
	ID3D11ShaderResourceView* velocityTextureSRV = FluidSetBufferGPU::getVelocityTextureSRV();
	PE::SA_Bind_Resource setVelocityTextureAction(*m_pContext, m_arena, DIFFUSE_TEXTURE_2D_SAMPLER_SLOT, t->m_samplerState, velocityTextureSRV);
	setVelocityTextureAction.bindToPipeline(&curEffect);
...
void EffectManager::bindFirstPassRTAsSampler(Handle hEffect, EGpuResourceSlot slot) {
    Effect* curEffect = hEffect.getObject<Effect>();
    if (!curEffect->m_isReady)
        return;
    //TextureGPU* t = m_firstPassTextureGPU.getObject<TextureGPU>();
	PE::SA_Bind_Resource bindTexture(*m_pContext, m_arena, slot, m_firstPassTextureGPU.m_samplerState, API_CHOOSE_DX11_DX9_OGL(m_firstPassTextureGPU.m_pShaderResourceView, t->m_pTexture, t->m_texture));
	bindTexture.bindToPipeline(curEffect);
    setFirstPassTextureAction = &bindTexture;
}

void EffectManager::unbindFirstPassRTAsSampler(Handle hEffect) {
	Effect* curEffect = hEffect.getObject<Effect>();
    setFirstPassTextureAction->unbindFromPipeline(curEffect);
	setFirstPassTextureAction = nullptr;
}
