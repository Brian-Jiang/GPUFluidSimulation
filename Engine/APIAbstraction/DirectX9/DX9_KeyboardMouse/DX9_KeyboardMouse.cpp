...
void DX9_KeyboardMouse::generateButtonEvents()
{
...
#endif
	{
		...
		if (isDragging) {
            ...
            float distance = sqrt((mousePosition.x - m_prevMousePos.x) * (mousePosition.x - m_prevMousePos.x) + (mousePosition.y - m_prevMousePos.y) * (mousePosition.y - m_prevMousePos.y));
			if (distance > 20.0f) {
				RECT rect;
				GetWindowRect(pWinApp->getWindowHandle(), &rect);
                float screenWidth = (float)(rect.right - rect.left);
                float screenHeight = (float)(rect.bottom - rect.top);

				Handle h("EVENT", sizeof(Event_Mouse_Left_Drag));
				Event_Mouse_Left_Drag* pEvent = new (h) Event_Mouse_Left_Drag;
				pEvent->m_current = Vector2((float)(mousePosition.x - rect.left) / screenWidth, (float)(mousePosition.y - rect.top) / screenHeight);
				m_pQueueManager->add(h, Events::QT_INPUT);
			}
		}
	}
