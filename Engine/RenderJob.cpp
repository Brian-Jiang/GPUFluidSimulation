...
void runDrawThreadSingleFrame(PE::GameContext &ctx)
{
	...
	if (!disableScreenSpaceEffects)
    {
		...
		// now it renders to glow texture
        //EffectManager::Instance()->setTextureAndDepthTextureRenderTargetForGlow();
		
		// render to first pass rt
        EffectManager::Instance()->setRTForFirstPass();
		...
		// the main rendering of scene objects
		// during the loop will switch to glow target
        ...
        // now it renders to separated glow texture using glow tex, render to m_glowSeparatedTextureGPU
        ...
        // use m_glowSeparatedTextureGPU as input, render to m_2ndGlowTargetTextureGPU
        EffectManager::Instance()->drawFirstGlowPass();
		EffectManager::Instance()->endCurrentRenderTarget();

        // from second glow target to FinishedGlowTargetTexture

        // use m_2ndGlowTargetTextureGPU as input, render to m_hFinishedGlowTargetTextureGPU
		...
		if (drawMotionBlur)
		{
            ...
            // use m_hFinishedGlowTargetTextureGPU as input, render to back buffer
		...
		//EffectManager::Instance()->drawFluidVelocity();
		//EffectManager::Instance()->endCurrentRenderTarget();
	}
	//else if (true) {
	//	// render to
