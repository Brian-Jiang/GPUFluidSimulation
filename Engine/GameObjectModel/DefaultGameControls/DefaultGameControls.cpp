...
void DefaultGameControls::handleKeyboardDebugInputEvents(Event *pEvt)
{
	...
	else if (Event_KEY_UP_HELD::GetClassId() == pEvt->getClassId())
	{
        ...
        FluidSetBufferGPU::setWillUpdate();
		...
		for (size_t i = 0; i < 15; i++) {
			...
			float xDir = (float)rand() / RAND_MAX * 100.0f;
			float yDir = (float)rand() / RAND_MAX * 100.0f;
			Vector2 force(xDir, yDir);

			FluidSetBufferGPU::updateForce(position, force, 12.0f);
		}
        
	}

	else if (Event_Mouse_Left_Drag::GetClassId() == pEvt->getClassId()) {
		Event_Mouse_Left_Drag* pRealEvent = (Event_Mouse_Left_Drag*)(pEvt);

		float xDir = (float)rand() / RAND_MAX * 100.0f;
		float yDir = (float)rand() / RAND_MAX * 100.0f;
		Vector2 force(xDir, yDir);
		//FluidSetBufferGPU::updateForce(pRealEvent->m_current, force, 8.0f);
		//UINT32 screenWidth = m_pContext->getGPUScreen()->getWidth();
  //      UINT32 screenHeight = m_pContext->getGPUScreen()->getHeight();

		CameraSceneNode* pcam = CameraManager::Instance()->getActiveCamera()->getCamSceneNode();
		float ndcX = (2.0f * pRealEvent->m_current.m_x) - 1.0f;
		float ndcY = -((2.0f * pRealEvent->m_current.m_y) - 1.0f);
        Vector3 nearPoint(ndcX, ndcY, 0.0f);
        Vector3 farPoint(ndcX, ndcY, 1.0f);
        Matrix4x4 invProj = pcam->m_viewToProjectedTransform.inverse();
        Matrix4x4 invView = pcam->m_worldToViewTransform.inverse();
        Vector3 nearWorld = invProj * nearPoint;
        Vector3 farWorld = invProj * farPoint;
        //nearWorld = nearWorld / nearWorld.m_w;
        //farWorld /= farWorld.m_w;
        nearWorld = invView * nearWorld;
        farWorld = invView * farWorld;
        Vector3 o = pcam->m_worldTransform.getPos();
        //Vector3 d = pcam->m_worldTransform.getN();
        Vector3 d = (farWorld - nearWorld);
        d.normalize();
        Vector3 x(0.0f, 0.5f, 0.0f);
        Vector3 n(0.01, 1.0f, 0.0f);
        float t = (x - o).dotProduct(n) / d.dotProduct(n);
        if (t > 0.1) {
            Vector3 wPosition = o + t * d;
			Vector2 nCoord((wPosition.m_x + 12.5) / 25.0, ((-wPosition.m_z + 12.5) / 25.0));
            FluidSetBufferGPU::updateForce(nCoord, force, 7.0f);
        }

		
		//Vector3 o = pcam->m_worldTransform.getPos();
		//Vector3 d = pcam->m_worldTransform.getN();
		//Vector3 x(0.0f, 0.01f, 0.0f);
		//Vector3 n(0.01, 1.0f, 0.0f);
  //      float t = (x - o).dotProduct(n) / d.dotProduct(n);
		//if (t > 0.1) {
  //          Vector3 wPosition = o + t * d;
		//	Vector2 nCoord((wPosition.m_x + 12.5) / 25.0, (wPosition.m_z + 12.5) / 25.0);
		//	FluidSetBufferGPU::updateForce(nCoord, force, 8.0f);
		//}

		//Handle h("EVENT", sizeof(Event_ROTATE_CAMERA));
		//Event_ROTATE_CAMERA* rotateCameraEvt = new(h) Event_ROTATE_CAMERA;
		//Vector3 relativeRotate(pRealEvent->m_current.getX(), pRealEvent->m_current.getY(), 0.0f);
		//rotateCameraEvt->m_relativeRotate = relativeRotate * Debug_Rotate_Speed * m_frameTime;
		//m_pQueueManager->add(h, QT_GENERAL);
	}
	else
