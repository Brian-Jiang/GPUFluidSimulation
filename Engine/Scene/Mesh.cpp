...
void Mesh::AddFluidSimulator() {
	m_hMyself.m_dbgName = "FluidSimulatorMesh";
	m_isFluidSimulator = true;
	SetFluidSurface();
}

void Mesh::SetFluidSurface() {
	m_effects[0].clear();
	m_effects[0].add(EffectManager::Instance()->getEffectHandle("Fluid_Surface_Tech"));
	m_hMyself.m_dbgName = "FluidSurface";
	m_isFluidSurface = true;
}
