...
void DrawList::setSwitchRenderTarget() {
    m_drawCallTypes[m_numDrawCalls - 1] = DrawCallType::SWITCH_RENDER_TARGET;
}

void DrawList::setDrawFullScreenQuad() {
	m_drawCallTypes[m_numDrawCalls - 1] = DrawCallType::DRAW_FULL_SCREEN_QUAD;
}

void DrawList::setUnbindRT() {
	m_drawCallTypes[m_numDrawCalls - 1] = DrawCallType::UNBIND_RT;
}

void DrawList::setRebindRT() {
	m_drawCallTypes[m_numDrawCalls - 1] = DrawCallType::REBIND_RT;
}
	...
	for (PrimitiveTypes::UInt32 i = 0; i < m_numDrawCalls; i++)
	{
		...
		else
		{
			PEASSERT(curDrawCallType == DrawCallType::COMPUTE || curDrawCallType == DrawCallType::COMPUTE_INDIRECT || 
				curDrawCallType == DrawCallType::SWITCH_RENDER_TARGET || curDrawCallType == DrawCallType::DRAW_FULL_SCREEN_QUAD ||
				curDrawCallType == DrawCallType::UNBIND_RT || curDrawCallType == DrawCallType::REBIND_RT,
		...
		else if (curDrawCallType == DrawCallType::COMPUTE || curDrawCallType == DrawCallType::COMPUTE_INDIRECT) // in case no vertex buffers were set, must be compute call
		{
...
#			endif
		...
		else if (curDrawCallType == DrawCallType::SWITCH_RENDER_TARGET) {
			//m_pContext->getGPUScreen()->setRenderTargetsAndViewportWithDepth()
			//EffectManager::Instance()->bindFirstPassRTAsSampler(hEffect, DIFFUSE_TEXTURE_2D_SAMPLER_SLOT);
			EffectManager::Instance()->setTextureAndDepthTextureRenderTargetForGlow();
		}
		else if (curDrawCallType == DrawCallType::DRAW_FULL_SCREEN_QUAD) {
            EffectManager::Instance()->drawFullScreenQuad();
		}
		else if (curDrawCallType == DrawCallType::UNBIND_RT) {
			EffectManager::Instance()->unbindDepth();
		}
		else if (curDrawCallType == DrawCallType::REBIND_RT) {
			EffectManager::Instance()->setTextureAndDepthTextureRenderTargetForGlow();
		}
		else
		{
