...
{
	if (pMeshCaller->m_isFluidSimulator) {  //  && !pMeshCaller->m_isFluidSurface
		if (!pDrawEvent || !pDrawEvent->m_drawOrder == EffectDrawOrder::Last) {  //  || !pDrawEvent->m_drawOrder == EffectDrawOrder::Last
			return;
		}

		FluidSetBufferGPU::handleFluidSimulatorDrawCalls(*m_pContext, pDrawList, pDrawEvent);

		// switch rt to back buffer
        pDrawList->beginDrawCallRecord("Fluid - SwitchRT");
        pDrawList->setSwitchRenderTarget();

		pDrawList->beginDrawCallRecord("Fluid - DrawFirstPass");
		pDrawList->setDrawFullScreenQuad();
		//return;
	}
	...
	for (PrimitiveTypes::UInt32 iEffect = 0; iEffect < pEffectsForRange->m_size; ++iEffect)
	{
		...
		for (int iRenderGroup = 0; iRenderGroup < numRenderGroups; ++iRenderGroup)
		{
			...
			for (PrimitiveTypes::UInt32 _iBoneSegment = 0; _iBoneSegment < numJointSegments; _iBoneSegment++)
			{
				...
				if (pMeshCaller->m_isFluidSurface) {
					FluidSetBufferGPU::handleFluidSurfaceResources(*m_pContext, pDrawList, pDrawEvent);
				}
