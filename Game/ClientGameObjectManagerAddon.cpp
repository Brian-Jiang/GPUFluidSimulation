...
void ClientGameObjectManagerAddon::createTargetNPC(Vector3 pos, int& threadOwnershipMask) {
	...
	//StringOps::writeToString("CharacterControl", evt.m_gunMeshPackage, 64);
	//StringOps::writeToString("", evt.m_patrolWayPoint, 32);
	auto targetNPC = createTargetNPC(&evt);
	//targetNPC->setAsFluidSurface();
	//targetNPC = createTargetNPC(&evt);
	targetNPC->setAsFluidSimulator();
}

TargetNPC* ClientGameObjectManagerAddon::createTargetNPC(Event_CreateTargetNPC* pTrueEvent) {
