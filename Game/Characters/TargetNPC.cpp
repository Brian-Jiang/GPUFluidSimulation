...
//			}
			...
			pSN->m_base.setPos(Vector3(0.0f, 0.5f, 0.0f));
			...
			pImrodMeshInst->initFromFile("cobbleplane.x_pplaneshape1_mesh2.mesha", "Default", m_pContext->m_gameThreadThreadOwnershipMask);
			//pImrodMeshInst->setDrawBoundingBox(pSN, pSN);
			//pImrodMeshInst->AddParticleEmitter();
			//pImrodMeshInst->AddFluidSimulator();
			...
			m_hMeshInstance = hImrodMeshInst;
		...
		void TargetNPC::setAsFluidSimulator() {
			MeshInstance* pImrodMeshInst = m_hMeshInstance.getObject<MeshInstance>();
			pImrodMeshInst->AddFluidSimulator();
		}

		void TargetNPC::setAsFluidSurface()
		{
			MeshInstance* pImrodMeshInst = m_hMeshInstance.getObject<MeshInstance>();
			pImrodMeshInst->AddFluidSurface();
		}
    }
}